...
def Query_Afternoon_Travel_Time(conn, HPHM):
    ...
    travel_time, sample_numbers, total_numbers = dataframe_res['AVG_T'][0], dataframe_res['SAM_NUM'][0], dataframe_res['SAM_NUM'].sum()    # 当需要统计：用于计算平均旅行时间的样本数时取消注释
...
def Pick_Up_Kids(df_res_c2):
    ...
    for i in range(len(HPHM)):
        travel_time, sample_numbers, total_numbers = Query_Afternoon_Travel_Time(conn, HPHM[i])    # 统计结果赋值给travel_time, sample_numbers
        TRAVEL_TIME_LIST.append(travel_time)
        SAMPLE_LIST.append(sample_numbers)
        TOTAL_NUM_LIST.append(total_numbers)

    DATA = {"HPHM": HPHM, "TRAVEL_TIME": TRAVEL_TIME_LIST, "SAMPLE_NUM": SAMPLE_LIST, "TOTAL_NUM":TOTAL_NUM_LIST}   # 将上面3个list转为字典
    df_res_c3 = pd.DataFrame(DATA,columns=['HPHM', 'TRAVEL_TIME', 'SAMPLE_NUM', 'TOTAL_NUM'])    # 转为dataframe
    df_res_c3 = df_res_c3.sort_values(by=['TRAVEL_TIME'], ascending=False).reset_index()    # 按平均旅行时间降序排列并重置索引
    df_res_c3 = df_res_c3[['HPHM', 'TRAVEL_TIME', 'SAMPLE_NUM', 'TOTAL_NUM']]    # 提取'HPHM', 'TRAVEL_TIME', 'SAMPLE_NUM'构成dataframe
